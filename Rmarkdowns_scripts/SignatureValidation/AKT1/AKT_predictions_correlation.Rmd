---
title: "AKT Signature Validation with ICBP drugs and with CCLE CNV data"
...
gfp_akt<-subset(feature,select=c(GFP.1:GFP.12,AKT.1:AKT.6))
gfp_akt_1 <- gfp_akt[apply(gfp_akt[,1:18]==0,1,mean) < 0.8,]###if not removed over 83%; error occurs in batch adjustment

gfp_akt_1_icbp<-merge_drop(gfp_akt_1,icbp,by=0)
#head(gfp_her2_1_icbp)
sub<-c(12,6,55)
pcaplot(mat = gfp_akt_1_icbp,sub=sub)
##############Batch adjustment is needed############
#with covariates
bat1<-as.matrix(cbind(c(colnames(gfp_akt_1),colnames(icbp)),c(rep(1,length(colnames(gfp_akt_1))),rep(2,length(colnames(icbp)))),c(rep(1,12),rep(2,6),rep(1,length(colnames(icbp))))))
#head(bat1)
#bat1[,2]
mod <- model.matrix(~as.factor(bat1[,3]))
combat_expr1<-ComBat(dat=gfp_akt_1_icbp, batch=bat1[,2], mod=mod, numCovs=NULL)
pcaplot(mat = combat_expr1,sub=sub)
#Combat without covariates
combat_expr2<-ComBat(dat=gfp_akt_1_icbp, batch=bat1[,2], mod=NULL, numCovs=NULL)
...
train_akt_cov<-combat_expr1[,1:18]
test_cov<-combat_expr1[,19:73]

train_akt<-combat_expr2[,1:18]
test<-combat_expr2[,19:73]

trainingLabel1=list(control = list(akt=1:12), akt = 13:18)
# dir.create("~/Desktop/tmp/akt")
# assign_easy(trainingData=train_akt, testData = test,trainingLabel1=trainingLabel1,g=150,out_dir_base="~/Desktop/tmp/akt")
# assign_easy(trainingData=train_akt, testData = test,trainingLabel1=trainingLabel1,g=250,out_dir_base="~/Desktop/tmp/akt/")
# assign_easy(trainingData=train_akt, testData = test,trainingLabel1=trainingLabel1,g=200,out_dir_base="~/Desktop/tmp/akt/")
# assign_easy(trainingData=train_akt, testData = test,trainingLabel1=trainingLabel1,g=100,out_dir_base="~/Desktop/tmp/akt/")
# assign_easy(trainingData=train_akt, testData = test,trainingLabel1=trainingLabel1,g=300,out_dir_base="~/Desktop/tmp/akt/")
# assign_easy(trainingData=train_akt_cov, testData = test_cov,trainingLabel1=trainingLabel1,g=150,cov=1,out_dir_base="~/Desktop/tmp/akt/")
# assign_easy(trainingData=train_akt_cov, testData = test_cov,trainingLabel1=trainingLabel1,g=25,cov=1,out_dir_base="~/Desktop/tmp/akt/")

setwd("~/Desktop/tmp/akt/")
...
Now correlating AKT predictions with ICBP drugs..
```{r, cache=TRUE}
...
akt_cnv<-ccle_cnv["AKT1",]
aktinterested_cnv<-akt_cnv[,grep("_BREAST",colnames(akt_cnv))]

colnames(aktinterested_cnv)<-gsub(pattern = "_BREAST",replacement = "",colnames(aktinterested_cnv)) 
#colnames(her2interested_cnv)

cnv = matrix(aktinterested_cnv["AKT1",],ncol=1)
rownames(cnv) = names(aktinterested_cnv["AKT1",])
...
for(i in 2:(length(colnames(data))+1))
{
  ...
  print(paste("pearson's correlation between AKT1 CNV and ",colnames(combined)[i]),sep='')
