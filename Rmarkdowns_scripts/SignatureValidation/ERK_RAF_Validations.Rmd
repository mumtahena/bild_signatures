...
cors_erk_preds_alldrugs_complete= cor(predicictionCols,drugs_55_cell_lines, use="na.or.complete",method= "spearman")
View(cors_erk_preds_alldrugs_complete)

cors_erk_preds_alldrugs_complete_df=data.frame(cors_erk_preds_alldrugs_complete)
...
# Not as good 
#erk_preds_all_cors_pair= cor(predicictionCols, drugs_55_cell_lines,use="pairwise.complete.obs",method= "spearman" )
...
Try with only complete observations (11 cell lines only)
```{r}
#try with only the complete cases (looks worse)
cellines_complete = rownames(pred_drug_erk_complete_only)
erk_sensitivity_complete= pred_drug_erk_complete_only$ERKi.II..FR180304
erk_sensitivity_complete
dim
erk_sensitivty_complete = cbind(cellines_complete,erk_sensitivity_complete)
class(erk_sensitivty_complete)
erk_sensitivty_complete[,2]

means_GI50_complete = colMeans(pred_drug_erk_complete_only[,22:111], na.rm=TRUE)
means_GI50

#find the mean of response to ERKi 
erk_mean_complete = means_GI50_complete["ERKi.II..FR180304."] 
erk_mean

#seperate the cell lines into sensitive and resistant to erk inhibitor 
erk_sensitive_complete= subset(erk_sensitivity_complete, erk_sensitivty_complete[,2] > 4.5, )
erk_sensitive_complete

erk_resistent_complete= subset(erk_sensitivity_complete, erk_sensitivty_complete[,2] < 4.5)
erk_resistent_complete

```

Looking at the predictions and drugs
```{r}

# 150 adaptive adaptive and ERK
pred_drug_erk[c(1,42)]
colnames(pred_drug_erk)

# 150 adaptive adaptive and Mek i
pred_drug_erk[c(1,2,3, 29)]
colnames(pred_drug_erk)

# 150 adaptive background only 
pred_drug_erk[c(2,42)]
colnames(pred_drug_erk)


pred_drug_erk[c(1,2, 107)] # had a high positive correlation
# for apap_apap100 and adap100, have really different predictions
```
