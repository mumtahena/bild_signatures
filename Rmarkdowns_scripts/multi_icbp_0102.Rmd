...
```{r include=FALSE}
setwd("~/Desktop/tmp/multi_icbp_expr_pc/")
...
Creating heatmaps
```{r }
multi<-read.table("~/Desktop/multipathway_preds.txt", sep='\t',row.names=1,header=1)
...
par(mfrow=c(2,1))
heatmap.2(as.matrix(comb[,22:25]),col=bluered,trace="none",main="Multipathway activity", ylab="ICBP Cell lines")
heatmap.2(as.matrix(comb[,26:29]),col=bluered,lmat=rbind( c(0, 3, 4), c(2,1,0 ) ), lwid=c(1.5, 4, 2 ), trace="none",main="Single pathway activity", ylab="ICBP Cell lines",scale="column")

comb_drug<-merge_drop(comb,drugs,by=0)
#plot(hclust(dist(comb_drug[,1:48]), method = "complete", members = NULL))

basal<-subset(comb_drug[,1:44],comb_drug$Transcriptional.subtype...ERBB2.status=="Basal")
heatmap.2(as.matrix(basal),col=bluered,lmat=rbind( c(0, 3, 4), c(2,1,0 ) ), lwid=c(1.5, 4, 2 ), trace="none",main="Basal Multi",scale="column")
...
heatmap.2(as.matrix(her),col=bluered,lmat=rbind( c(0, 3, 4), c(2,1,0 ) ), lwid=c(1.5, 4, 2 ), trace="none",main="HER2 Multi",scale="column")
claudin<-subset(comb_drug[,1:44],comb_drug$Transcriptional.subtype...ERBB2.status=="Claudin-low")
heatmap.2(as.matrix(claudin),col=bluered,lmat=rbind( c(0, 3, 4), c(2,1,0 ) ), lwid=c(1.5, 4, 2 ), trace="none",main="Claudin Multi",scale="column")
luminal<-subset(comb_drug[,1:44],comb_drug$Transcriptional.subtype...ERBB2.status=="Luminal")
heatmap.2(as.matrix(luminal),col=bluered,lmat=rbind( c(0, 3, 4), c(2,1,0 ) ), lwid=c(1.5, 4, 2 ), trace="none",main="Claudin low Multi",scale="column")
dev.off()
```
