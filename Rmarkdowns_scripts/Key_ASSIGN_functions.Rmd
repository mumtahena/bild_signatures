...
pcaplot<-function(mat,sub,scale=T){
  if(sum(sub)!=length(mat)){
    ...
    for(i in 1:length(sub)){
      ...
      print(i)
      if(i==1){
        points(pca_mat$x[1:sub[i]],pca_mat$x[1:sub[i],2],col=i+1,pch=i+1)
        }
      else if(i==length(sub)){
         points(pca_mat$x[index:length(rownames(pca_mat$x))],pca_mat$x[index:length(rownames(pca_mat$x)),2],col=i+1,pch=i+1)
         }
      else{
        points(pca_mat$x[index:index+sub[i]],pca_mat$x[index:index+sub[i],2],col=i+1,pch=i+1)
        }
       index=index+sub[i]
      }
  }
